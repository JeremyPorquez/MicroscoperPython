...
try : from ctypes import c_long, c_buffer, c_float, windll, pointer
except : from ctypes import c_long, c_buffer, c_float, pointer
...
class APTMotor():
    def __init__(self, SerialNum=None, HWTYPE=31, verbose=False):
        ...
        if sys.maxsize > 2**32:
            dllname = os.path.join(os.path.dirname(__file__), r'dlls\APT64.dll')
        else :
            dllname = os.path.join(os.path.dirname(__file__), r'dlls\APT32.dll')
        print(dllname)
        ...
        if SerialNum is not None:
            ...
            if self.verbose: print("No serial, please set SerialNumber")
